...
public class PurchaseFormController {
	...
	@GetMapping("/setPrintIp")
	public Result<?> setPrintIp(String ip,String paper,String addressid) throws ERPBizException {
		UserInfo userinfo = UserInfoContext.get();
		purchaseFormService.setPrintIp(userinfo,ip,paper,addressid);
		return Result.success();
	}

	@GetMapping("/getPrintIpByAddress")
	public Result<PurchaseFormPrintIP> getPrintIpByAddress(String addressid) throws ERPBizException {
		UserInfo userinfo = UserInfoContext.get();
		return Result.success(purchaseFormService.getPrintIpByAddress(userinfo,addressid));
	}
	@GetMapping("/getPrintIpByWarehouse")
	public Result<PurchaseFormPrintIP> getPrintIpByWarehouse(String warehouseid) throws ERPBizException {
		UserInfo userinfo = UserInfoContext.get();
		Warehouse warehouse = iWarehouseService.getById(warehouseid);
		return Result.success(purchaseFormService.getPrintIpByAddress(userinfo,warehouse.getAddressid()));
	}
	@GetMapping("/getPrintIp")
	public Result<PurchaseFormPrintIP> getPrintIp(String addressid) throws ERPBizException {
		UserInfo userinfo = UserInfoContext.get();
		return Result.success(purchaseFormService.getPrintIp(userinfo));
	}
	@GetMapping("/clearRec")
	...
	public Result<IPage<Map<String, Object>>> getListData(@RequestBody PurchaseFormDTO dto) throws BizException {
		...
		Map<String, Object> param = new HashMap<String, Object>();
		  ...
		  String search = dto.getSearch();
			...
			if (GeneralUtil.isNotEmpty(dto.getEntryid())) {
				param.put("entryid", dto.getEntryid());
			}
		 ...
		 if(dto.getNeedinventory()!=null&&dto.getNeedinventory()==true&& list.getRecords() != null && list.getSize() > 0) {
				for (Map<String, Object> item : list.getRecords()) {
					String warehosueid=item.get("warehouseid").toString();
					String materialid=item.get("materialid").toString();
					InventoryVo inv = iInventoryService.getInventory(materialid, warehosueid, userinfo.getCompanyid());
					item.putAll(BeanUtil.beanToMap(inv));
					item.put("materialid",materialid);
					item.put("warehosueid",warehosueid);
				}
			}
		 return Result.success(list);
	}
	...
	public Result<List<PurchaseFormReceiveVo>> getReceiveReport(@RequestBody ReceiveReportDTO condition){
		...
		if (StrUtil.isNotBlank(condition.getIsConsumable())) {
			param.put("isConsumable", condition.getIsConsumable());
		}else {
			param.put("isConsumable", null);
		}
		if (StrUtil.isNotBlank(condition.getRunid())) {
			param.put("runid", condition.getRunid());
		}else {
			param.put("runid", null);
		}
		Page<PurchaseFormReceiveVo> page=condition.getPage();
	    ...
	    public  void getReceiveReportExcel(@ApiParam("仓库ID")@RequestParam String warehouseid,
			...
			if (StrUtil.isNotBlank(isConsumable)) {
				param.put("isConsumable",isConsumable);
			}else {
				param.put("isConsumable", null);
			}
			try {
		...
		public Result<Map<String, Object>> paymentAction(@RequestBody PaymentSaveDTO dto) {
			...
			try {
				...
				result=purchaseFormService.getTraceDetailMap(entryid, shopid, "pay",paytype);
			}catch(Exception e) {
				e.printStackTrace();
				throw e;
			}finally{
		...
		public int sysAutoPay(UserInfo user, List<String> ids,String type, String acct) {
			...
			for(Entry<String, BigDecimal> entryProject:projectMap.entrySet()) {
	...
	public Result<Map<String, Object>> updateNoticeFormAction(String formid,String notice,String entryids){
		...
		@SystemControllerLog(value = "重新提交采购单")
		@GetMapping("/reSubmit")
		public Result<Map<String, Object>> reSubmitAction(String id){
			UserInfo userinfo = UserInfoContext.get();
			Map<String, Object> result = purchaseFormService.reSubmit(userinfo,id);
			return Result.success(result);
		}
	...
	public Result<Map<String,Object>> receiveInfoAction(String id){
		UserInfo userinfo = UserInfoContext.get();
		return Result.success(this.iPurchaseFormReceiveService.receiveInfo(userinfo,id));
	}
				...
				if (file != null) {
					try {
						...
						for(Entry<String, PurchaseFormEntry> item:entryMap.entrySet()) {
							iPurchaseFormPaymentService.updatePayment(item.getValue().getPayList(),item.getValue(),null, user);
				        }
			...
			new Thread(new Runnable() {
				...
				public void run() {
					...
					try {
						...
						purchaseFormEntryService.downloadTimeList(workbook, dto);
						ByteArrayOutputStream fOut=new ByteArrayOutputStream();
						workbook.write(fOut);
						fOut.flush();
						fOut.close();
						workbook.close();
						downloadReportService.updateContent(fOut.toByteArray(), user);
					} catch ( IOException e) {
						// TODO Auto-generated catch block
						e.printStackTrace();
						downloadReportService.updateError(e, user);
					}
				}
			}).start();
			// 创建新的Excel工作薄
